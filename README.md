# VRASED+: VRASED with Verifier Authentication

**THIS REPOSITORY IS AN UPGRADED VERSION OF [VRASED](https://github.com/sprout-uci/vrased) WHICH INCLUDES VERIFIER AUTHENTICATION. NOTE THAT THIS AN ADDITIONAL FEATURE AND IS NOT A PART OF VRASED FORMAL VERIFICATION.**

## Changes include:

1. Stores a persistent counter in flash (PMEM) at location *0xffc0* which can be updated only by VRASED.
2. VRASED software (see vrased/sw-att/wrapper.c) in ROM supports verifier authentication using the counter.
3. Verification of VRASED additional hardware for counter storage is included in verification_specs/ltl_specs.smv.
